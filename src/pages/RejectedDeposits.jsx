...
const RejectedDeposits = () => {
  ...
  const filteredDeposits = deposits.filter(deposit =>
    deposit.gateway.toLowerCase().includes(searchQuery.toLowerCase()) ||
    deposit.user.toLowerCase().includes(searchQuery.toLowerCase()) ||
    deposit.amount.includes(searchQuery) ||
  ...
  const currentDeposits = filteredDeposits.slice(startIndex, startIndex + entriesPerPage);
  ...
  return (
    <div className="bg-gray min-h-screen  font-sans antialiased">
      {/* Header */}
      <div className="mb-2">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
          <h1 className="text-3xl font-bold text-gray-800 mb-2"> Rejected Deposits</h1>
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <span>Dashboard</span>
            <ChevronRight className="w-4 h-4" />
            <span>Deposits</span>
            <ChevronRight className="w-4 h-4" />
            <span className="text-gray-700 font-medium">Rejected Deposits</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="relative">
              <input
                ...
                className="border border-gray-300 rounded-md px-4 py-2 pr-10 text-sm w-64 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
        ...
        <div className="overflow-x-auto max-w-233">
                ...
                >
                  <td className="p-4 text-nowrap">
                    <div className="flex items-center gap-3">
                      {getGatewayIcon(deposit.gateway)}
                  ...
                  <td className="p-4 text-nowrap">
                    <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full cursor-pointer ${getStatusColor(deposit.status)}`}>
                      {deposit.status}
                    </span>
                  </td>
                  <td className="p-4 text-nowrap">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center p-6 border-t border-gray-200 gap-4">
          <div className="text-sm text-gray-600">
            Showing {startIndex + 1} to {Math.min(startIndex + entriesPerPage, filteredDeposits.length)} entries
          </div>
            ...
            <div className="flex flex-col sm:flex-row">
              ...
              <div className="flex-1 p-4 bg-white">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-bold text-gray-800">Deposit Via Bank Transfer</h3>
                  <button
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  <div className="flex justify-between items-center py-2 border-b border-gray-100">
                    ...
                    <span className="text-gray-800 font-semibold text-sm">{selectedDeposit.initiated}</span>
                  </div>
                  
                  <div className="flex justify-between items-center py-2 border-b border-gray-100">
                    <span className="text-gray-600 text-sm">Tx Number</span>
                    <span className="text-blue-600 font-semibold text-sm">{selectedDeposit.gateway.split(' ')[1] || 'XT5C1WA77S4X'}</span>
                  </div>
                  
                  <div className="flex justify-between items-center py-2 border-b border-gray-100">
                    <span className="text-gray-600 text-sm">Username</span>
                    <span className="text-blue-600 font-semibold text-sm">@{selectedDeposit.user.toLowerCase().replace(' ', '')}</span>
                  </div>
                  
                  <div className="flex justify-between items-center py-2 border-b border-gray-100">
                    <span className="text-gray-600 text-sm">Method</span>
                    <div className="flex items-center gap-2">
                      {getGatewayIcon(selectedDeposit.gateway)}
                      <span className="text-gray-800 font-semibold text-sm">{selectedDeposit.gateway.split(' ')[0]}</span>
                    </div>
                  </div>
                  
                  <div className="flex justify-between items-center py-2 border-b border-gray-100">
                    <span className="text-gray-600 text-sm">Amount</span>
                    <span className="text-gray-800 font-semibold">{selectedDeposit.conversion}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center py-2">
                    ...
                    <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full ${
                      selectedDeposit.status.toLowerCase() === 'success' ? 'bg-green-500 text-white' :
                      selectedDeposit.status.toLowerCase() === 'failed' ? 'bg-red-500 text-white' :
              ...
              {/* Right Side - User Withdraw Information */}
              <div className="w-full sm:w-64 bg-gray-50 p-4 border-l border-gray-200">
                <h3 className="text-base font-bold text-gray-800 mb-4">User Withdraw Information</h3>
